 ...
 * @return {*}
 */
export function getDepKeys(depObj) {
...
export function useCustomMemo(depObj, memoFunc, errorCb) {
 ...
 * @return {*}
 */
export function useCustomCallback(depObj, memoFunc, errorCb) {
    const deps = getDepKeys(depObj);
    const errorCbR = useBindRef(errorCb);
    // eslint-disable-next-line
 ...
 * @param {(err: Error) => ReturnType<EffectCallback>} [errorCb]
 */
export function useCustomEffect(depObj, cb, ignoreFirst = false, errorCb) {
    const deps = getDepKeys(depObj);
 ...
 * @param {(err: Error) => ReturnType<EffectCallback>} [errorCb]
 */
export function useCustomLayoutEffect(depObj, cb, ignoreFirst = false, errorCb) {
    const deps = getDepKeys(depObj);
    const cbR = useBindRef(cb);
    const changedR = useRef(!ignoreFirst);
    const errorCbR = useBindRef(errorCb);
 ...
 * @param {(err: Error) => ReturnType<EffectCallback>} [errorCb]
 */
export function usePartialEffect(depObj, relObj, cb, ignoreFirst = false, errorCb) {
    const deps = getDepKeys(depObj);
    const cbR = useBindRef(cb);
    const errorCbR = useBindRef(errorCb);
